 ...
 * 0.4   KMC 12/16/2022 - added sort functionality, used previous class code for insertion sort
 * 
 *                      - FINAL NOTES, A lot of this code was from in class stuff, so thank you for that.
 *                      - Most of it was transferring items to and from one data structure to another, which when the
 *                        classes are used right, actually becomes really simple and easy. THANK YOU! LOOKING FORWARD TO
 *                        NEXT SEMESTER!
 * *******************************************************************/
using System;
using System.IO;
...
namespace KMCStacksAndQueuesFinal
{
    public partial class FormStacksAndQueues : Form
    {
        ...
        /// <summary>
        /// This method selects a file from the users file explorer and returns the file ||| Took from class code
        /// </summary>
        /// <returns></returns>
        ...
        /// <summary>
        /// This button sorts the entire queue with the smallest value at the front of the queue
        /// USED THIS WEBSITE TO ADD ITEMS FROM AN ARRAY TO STACK / QUEUE https://kodify.net/csharp/queue/enqueue/
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void buttonQueueSort_Click(object sender, EventArgs e)
        {
            int count = 0;              
            string number;

            // array to use for sort
            int[] numberArray = new int[numberQueue.Count];

            // while the queue is greater than 0, convert the values and add them to the array
            while (numberQueue.Count > 0)
            {
                number = numberQueue.Dequeue();                
                numberArray[count] = Convert.ToInt32(number);
                count++;
            }

            // sort the array
            KMCInsertionSort.Sort(numberArray);

            // add the values back the the queue
            foreach (int num in numberArray)
            {              
                numberQueue.Enqueue(num.ToString());
                DisplayQueue(numberQueue, listBoxQueue);
            }
        }
        #endregion events
        ...
        /// <summary>
        /// This button sorts the values in the stack with the smallest number on top
        /// USED THIS WEBSITE TO ADD ITEMS FROM AN ARRAY TO STACK / QUEUE https://kodify.net/csharp/queue/enqueue/
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void buttonStackSort_Click(object sender, EventArgs e)
        {
            int count = 0;
            string number;

            // array of the stack values to an array to sort
            int[] numberArray = new int[numberStack.Count];
            ...
            // sort the array
            KMCSelectionSort.Sort(numberArray);

            // add the values back to the stack
            foreach (int num in numberArray)
            {
                numberStack.Push(num.ToString());
                DisplayStack(numberStack, listBoxStack);
            }
        }
        #endregion events
